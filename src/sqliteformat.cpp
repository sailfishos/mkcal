                                     ...
                                     sqlite3_stmt *stmt10, sqlite3_stmt *stmt11 )
{
  ...
  if ( dbop == DBDelete || dbop == DBUpdate ) {
    ...
    if ( !rowid ) {
      kError() << "failed to select rowid of incidence" << incidence->uid()
  ...
  if ( dbop == DBInsert || dbop == DBUpdate ) {
    ...
    if ( ( incidence->type() == Incidence::TypeEvent ) ||
      ...
      if ( todo->hasStartDate() ) {
        secs = d->mStorage->toOriginTime( todo->dtStart( true ) );
        sqlite3_bind_int64( stmt1, index, secs );
        tzStart = todo->dtStart( true ).timeZone().name().toUtf8();
        sqlite3_bind_text( stmt1, index, tzStart.constData(), tzStart.length(), SQLITE_STATIC );
      } else {
        sqlite3_bind_int( stmt1, index, 0 );
      ...
      if ( todo->hasDueDate() ) {
        ...
        sqlite3_bind_int64( stmt1, index, secs );
        tzEnd = todo->dtDue( true ).timeZone().name().toUtf8();
        sqlite3_bind_text( stmt1, index, tzEnd.constData(), tzEnd.length(), SQLITE_STATIC );
      } else {
        ...
        if ( todo->hasStartDate() ) {
          secs = d->mStorage->toOriginTime( todo->dtStart( true ) );
          sqlite3_bind_int64( stmt1, index, secs );
          tzEnd = todo->dtStart( true ).timeZone().name().toUtf8();
          sqlite3_bind_text( stmt1, index, tzEnd.constData(), tzEnd.length(), SQLITE_STATIC );
        } else {
          sqlite3_bind_int( stmt1, index, 0 );
          sqlite3_bind_text( stmt1, index, "", 0, SQLITE_STATIC );
    ...
    sqlite3_bind_int( stmt1, index, int( incidence->secrecy() ) ); // NOTE
    ...
    sqlite3_bind_int( stmt1, index, int( incidence->status() ) ); // NOTE
    ...
    sqlite3_bind_text(
      stmt1, index, relatedtouid.constData(), relatedtouid.length(), SQLITE_STATIC );
    ...
    if ( incidence->type() == Incidence::TypeTodo ) {
      ...
      if ( todo->isCompleted() ) {
        ...
        secs = d->mStorage->toOriginTime( todo->completed() );
        sqlite3_bind_int64( stmt1, index, secs );
        tzComp = todo->completed().timeZone().name().toUtf8();
                                                  ...
                                                  DBOperation dbop, sqlite3_stmt *stmt )
{
  ...
  if ( dbop == DBInsert ) {
    ...
    sqlite3_bind_text(
      stmt, index, parametersba.constData(), parametersba.length(), SQLITE_STATIC );
  }
                                          ...
                                          sqlite3_stmt *stmt1, sqlite3_stmt *stmt2 )
{
  ...
  if ( success && dbop != DBDelete ) {
    ...
    for ( dt = dateList.constBegin(); dt != dateList.constEnd(); ++dt ) {
      if ( !modifyRdate( rowid, type, KDateTime( (*dt), QTime( 00, 00, 00 ) ).toClockTime(),
                         ( dbop == DBUpdate ? DBInsert : dbop ), stmt2 ) ) {
        kError() << "failed to modify rdates for incidence" << incidence->uid();
        success = false;
      }
                                              ...
                                              sqlite3_stmt *stmt1, sqlite3_stmt *stmt2 )
{
  ...
  if ( dbop == DBUpdate || dbop == DBDelete ) {
    // In Update always delete all first then insert all
    // In Delete delete with uid at once
    if ( !modifyRecursive( rowid, 0, DBDelete, stmt1, 1 ) ) {
                                               ...
                                               QString &notebook )
{
  ...
  if ( rv == SQLITE_ROW ) {
    ...
    if ( type == "Event" ) {
      ...
      timezone = QString::fromUtf8( (const char *)sqlite3_column_text( stmt1, 6 ) );
    ...
    incidence->setCreated( d->mStorage->fromOriginTime( sqlite3_column_int64( stmt1, index++ ) ) );
    ...
    if ( stmt2 && !d->selectCustomproperties( incidence, rowid, stmt2 ) ) {
      kWarning() << "failed to get customproperties for incidence" << incidence->uid()
                 << "notebook" << notebook;
    }
    if ( stmt3 && !d->selectAttendees( incidence, rowid, stmt3 ) ) {
      kError() << "failed to get attendees for incidence" << incidence->uid()
               << "notebook" << notebook;
    }
    if ( stmt4 && !d->selectAlarms( incidence, rowid, stmt4 ) ) {
      kError() << "failed to get alarms for incidence" << incidence->uid()
               << "notebook" << notebook;
    }
    if ( stmt5 && !d->selectRecursives( incidence, rowid, stmt5 ) ) {
      kError() << "failed to get recursive for incidence" << incidence->uid()
               << "notebook" << notebook;
    }
    if ( stmt6 && !d->selectRdates( incidence, rowid, stmt6 ) ) {
      kError() << "failed to get rdates for incidence" << incidence->uid()
               << "notebook" << notebook;
    }
  }
  ...
  do {
    ...
    if ( rv == SQLITE_ROW ) {
      ...
      if ( type == SqliteFormat::RDate || type == SqliteFormat::XDate ) {
        kdt = mStorage->fromOriginTime( date );
        ...
        timezone = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 3 ) );
                                              ...
                                              sqlite3_stmt *stmt )
{
  ...
  do {
    ...
    if ( rv == SQLITE_ROW ) {
      ...
      recurrule->setDuration( sqlite3_column_int( stmt, 5 ) );  // count
      // Frequency
      recurrule->setFrequency( sqlite3_column_int( stmt, 6 ) ); // interval-field
...
#define readSetByList( field, setfunc )                                 \
      ...
      // and SETPOS are standard int lists, so we can treat them with the
      // same macro
      readSetByList( 7, setBySeconds );
      readSetByList( 8, setByMinutes );
      readSetByList( 9, setByHours );
      readSetByList( 12, setByMonthDays );
      readSetByList( 13, setByYearDays );
      readSetByList( 14, setByWeekNumbers );
      readSetByList( 15, setByMonths );
      readSetByList( 16, setBySetPos );
...
#undef readSetByList
      ...
      wdList.clear();
      byList.clear();
      by = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 10 ) );
      by2 = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 11 ) );
      if ( !by.isEmpty() ) {
        byL = by.split( ' ' );
        if ( !by2.isEmpty() ) {
          byL2 = by2.split( ' ' );
      ...
      // Week start setting
      recurrule->setWeekStart( sqlite3_column_int( stmt, 17 ) );
...
bool SqliteFormat::Private::selectAlarms( Incidence::Ptr incidence, int rowid, sqlite3_stmt *stmt )
{
  ...
  do {
    ...
    if ( rv == SQLITE_ROW ) {
      ...
      timezone = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 7 ) );
      kdt = mStorage->fromOriginTime( date, timezone );
      if ( kdt.isValid() ) {
        ialarm->setTime( kdt );
      }
      ...
      QString description =  QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 8 ) );
      QString attachments =  QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 9 ) );
      QString summary = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 10 ) );
      QString addresses = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 11 ) );
      ...
      QString properties = QString::fromUtf8( (const char *)sqlite3_column_text( stmt, 12 ) );
      ...
      ialarm->setEnabled( (bool)sqlite3_column_int( stmt, 13 ) );
    }
