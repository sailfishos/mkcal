...
bool SqliteFormat::purgeAllComponents(const QString &notebook)
{
    ...
    const QByteArray n = notebook.toUtf8();
    ...
    query = SELECT_ROWID_FROM_COMPONENTS_BY_NOTEBOOK;
    qsize = sizeof(SELECT_ROWID_FROM_COMPONENTS_BY_NOTEBOOK);

    SL3_prepare_v2(d->mDatabase, query, qsize, &stmt, NULL);
...
QDateTime SqliteFormat::deleted(const KCalendarCore::Incidence &incidence)
{
    bool ok;
